...
use foundry_compilers_new::{
    artifacts, artifacts::output_selection::{OutputSelection, ContractOutputSelection}, solc::SolcLanguage,
};
use serde::{Deserialize, Serialize};
use serde_json::Value;
use std::{collections::BTreeMap, path::Path, sync::Arc};

/// Create output selection without evm.gasEstimates to avoid compiler errors
/// with newer opcodes like CLOAD
fn create_safe_output_selection() -> OutputSelection {
    let contract_outputs = vec![
        ...
        "devdoc".to_string(),
        "userdoc".to_string(),
        // Note: Explicitly excluding "evm.gasEstimates" to prevent errors
    ];
    
    BTreeMap::from([
        ("*".to_string(), BTreeMap::from([
            ("*".to_string(), contract_outputs),
...
impl evm_compilers::CompilerInput for SolcInput {
    fn normalize_output_selection(&mut self, _version: &semver::Version) {
        self.0.settings.output_selection = create_safe_output_selection();
    }
...
impl evm_compilers::EvmCompiler for SolcCompiler {
    ...
    async fn compile(
        ...
        tracing::debug!("Compiling with solc version: {}", compiler_version);
        tracing::debug!("Input EVM version: {:?}", input.0.settings.evm_version);
        let output = solc
        ...
        tracing::debug!("output_value JSON (compressed): {}", serde_json::to_string(&output_value).unwrap_or_else(|_| "Failed to serialize".to_string()));
