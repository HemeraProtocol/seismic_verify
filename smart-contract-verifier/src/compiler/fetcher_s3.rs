...
impl<Ver: Version> VersionsFetcher for S3VersionFetcher<Ver> {
    ...
    async fn fetch_versions(&self) -> Result<Self::Versions, Self::Error> {
        tracing::info!("📦 Fetching Solidity versions from S3 bucket: {}", self.bucket.name);
        let folders = self
            .bucket
...
impl<Ver: Version> S3Fetcher<Ver> {
    pub async fn new(
        ...
        tracing::info!("🚀 Creating S3Fetcher with bucket: {}, region: {}", 
        ...
        tracing::info!("✅ S3Fetcher initialized successfully!");
        Ok(S3Fetcher {
            bucket,
            folder,
            versions,
