    ...
    <Ver as FromStr>::Err: std::fmt::Display,
{
    let fetcher: Arc<dyn Fetcher<Version = Ver>> = match fetcher_settings {
        FetcherSettings::List(list_settings) => {
            println!("🔗 Using List fetcher with URL: {}", list_settings.list_url);
            tracing::info!("🔗 Using List fetcher with URL: {}", list_settings.list_url);
            Arc::new(
                ListFetcher::new(
        ...
        FetcherSettings::S3(s3_settings) => {
            println!("☁️ Using S3 fetcher with bucket: {} (region: {:?}, endpoint: {:?})", 
                s3_settings.bucket, s3_settings.region, s3_settings.endpoint);
            tracing::info!("☁️ Using S3 fetcher with bucket: {} (region: {:?}, endpoint: {:?})", 
                s3_settings.bucket, s3_settings.region, s3_settings.endpoint);
            Arc::new(
                S3Fetcher::new(
